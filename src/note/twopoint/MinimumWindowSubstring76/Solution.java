...
public class Solution {
    ...
    //https://leetcode.com/discuss/82838/java-easy-understand-solution-with-o-n-complexity
    ...
    // i iterative once, j iterative once
    ...
    public String minWindow(String s, String t) {
        ...
        // start and end are used to record the answer's boundaries
        int start = 0, end = 0;
        int lenS = s.length();
        int lenT = t.length();
        int min = lenS + 1;
        for (int i = 0, j = 0; j < lenS; j++) {
            char c = s.charAt(j);
            // idx represents the current value of s.charAt(j)
            int idx = c > 'Z' ? c - 'a' + 26 : c - 'A';
            ++state[idx];
            if (isContain(tmap, state)) {
                ...
                i = i + 1;
                while (j + 1 - i >= lenT) {
                    c = s.charAt(i - 1);  // decrease
                    //now the idx represents the current value of s.charAt(i - 1)
                    idx = c > 'Z' ? c - 'a' + 26 : c - 'A';
                    state[idx]--;
                    if (isContain(tmap, state)) {
                        if (j + 1 - i < min) {
