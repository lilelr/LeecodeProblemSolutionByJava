package note.dp.LongestIncreasingSubsequence300;
...
public class Solution {
    ...
    public int lengthOfLISByDP(int[] nums) {
        ...
        int[] result = new int[n];// record the longest increasing subsequence
        result[0] = nums[0];
        for (int i = 1; i < n; i++) {
            ...
            if (currentElement <= result[0]) {
                ...
                result[position] = currentElement;// simultaneously replace the origin item result[position]
            }
        }
        return len;
    }
     ...
     * @param start   start
     * @param end   end
     * @return   the right index to insert
     */
